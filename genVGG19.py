...
class VGG16(nn.Module):
    def __init__(self, mask = None):
        ...
        if self.initialize is True:
            ...
            self.layer = self.layer + 1
        self.conv9 = nn.Conv2d(512, 512, kernel_size = 3, padding="same")
        if self.initialize is True:
            self.mask[self.layer] = torch.ones(512).to(device)
            self.layer = self.layer + 1
        ...
        # 512 X 3
        self.conv11 = nn.Conv2d(512, 512, kernel_size = 3, padding="same")
        if self.initialize is True:
            self.mask[self.layer] = torch.ones(512).to(device)
            self.layer = self.layer + 1
...
class VGG(nn.Module):
    def __init__(self, mask = None):
        ...
        if self.initialize is True:
            self.mask[self.layer] = torch.ones(512).to(device)
            self.layer = self.layer + 1
        self.conv14 = nn.Conv2d(512, 512, kernel_size = 3, padding="same")
        if self.initialize is True:
            self.mask[self.layer] = torch.ones(512).to(device)
            self.layer = self.layer + 1
        self.conv15 = nn.Conv2d(512, 512, kernel_size = 3, padding="same")
        if self.initialize is True:
            self.mask[self.layer] = torch.ones(512).to(device)
            self.layer = self.layer + 1
        self.conv16 = nn.Conv2d(512, 512, kernel_size = 3, padding="same")
        self.bn5 = nn.BatchNorm2d(num_features=512)
        if self.initialize is True:
            self.mask[self.layer] = torch.ones(512).to(device)
            self.layer = self.layer + 1
...
#                     layers.append(initial_array4)
        ...
        printgraph(cache, 'vgg19_evaluate' + str(trial), numtrials = trial+1)
...
modelname = 'cifar10_vgg19_evaluate'
