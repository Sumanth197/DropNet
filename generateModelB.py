...
def add_filter_mask(mask, model, activationarray, layer, initialize=True):
    ...
    # model2 = F.adaptive_avg_pool2d(model, 64)
    ...
    # print("model shape", model.shape, mask[layer].shape)
...
class MyModel(nn.Module):
    ...
    def __init__(self, mask = None):
        ...
        if self.initialize is True:
            self.mask[self.layer] = torch.ones(32).to(device)
        ...
        self.fc = nn.Linear(64 * 7 * 7, 10)  # Assuming input image size of 28 * 28
    ...
    def forward(self, x):
        ...
        global_avg_pool = nn.AdaptiveAvgPool2d((1, 1))
        output_tensor = global_avg_pool(x)
        model2 = output_tensor.view(output_tensor.size(0), -1)
        model2 = model2 * self.mask[self.layer].reshape(model2.shape[1:])
        self.activationarray.append(model2)
        x = x * self.mask[self.layer].reshape(x.shape[1], 1, 1)
        self.layer = self.layer + 1
        
        # x, activationarray, layer, self.mask = add_filter_mask(self.mask, x, activationarray, layer, initialize=self.initialize)
        x = self.pool(x)
        # print("3", x.shape)
...
cache = generatemodel(criterion, patience = 5, num_epochs = 100, 
                      numtrials = 3, percentile = 0.2, printvalue = True, 
                      printmask = True, printactivation = True)

# modelname = 'mnist_dense40_dense40_evaluate'
modelname = 'mnist_conv32_conv64_evaluate'
